...
class Decision_Polls_Admin {
	...
	private function process_bulk_actions() {
		...
		if ( ! isset( $_POST['bulk_action'] ) || $_POST['bulk_action'] === '-1' || empty( $_POST['poll_ids'] ) || ! is_array( $_POST['poll_ids'] ) ) {
			return;
		}
		...
		if ( ! isset( $_POST['decision_polls_bulk_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['decision_polls_bulk_nonce'] ) ), 'decision_polls_bulk_action' ) ) {
			wp_die( __( 'Security check failed.', 'decision-polls' ) );
		}
		...
		if ( $action === 'delete' ) {
			...
			if ( $success_count > 0 ) {
				...
				add_settings_error(
					'decision_polls',
					'polls_deleted',
					sprintf( _n( '%d poll deleted successfully.', '%d polls deleted successfully.', $success_count, 'decision-polls' ), $success_count ),
					'success'
				);
			}
			...
			if ( $error_count > 0 ) {
				...
				add_settings_error(
					'decision_polls',
					'polls_delete_failed',
					sprintf( _n( 'Failed to delete %d poll.', 'Failed to delete %d polls.', $error_count, 'decision-polls' ), $error_count ),
					'error'
				);
			}
		}
					...
					<?php foreach ( $polls as $poll ) : ?>
						...
						<tr>
							<td class="check-column">
								<label class="screen-reader-text" for="cb-select-<?php echo esc_attr( $poll_id ); ?>"><?php echo esc_html( sprintf( __( 'Select %s', 'decision-polls' ), $poll['title'] ) ); ?></label>
