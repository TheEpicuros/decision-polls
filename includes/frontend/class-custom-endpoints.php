...
class Custom_Endpoints {
	...
	public static function add_rewrite_rules() {
		...
		// Add rewrite rule for poll results viewing.
		add_rewrite_rule(
			'^poll/([0-9]+)/results/?$',
			'index.php?poll_id=$matches[1]&show_results=1',
			'top'
		);
	...
	public static function handle_template_redirect() {
		...
		if ( get_query_var( 'poll_id' ) ) {
			...
			if ( $poll_id > 0 ) {
				...
				// First check if this is a results-specific URL.
				if ( isset( $_SERVER['REQUEST_URI'] ) ) {
					$request_uri = sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) );
					if ( strpos( $request_uri, '/results/' ) !== false ) {
						$show_results = true;
					}
				} elseif ( get_query_var( 'show_results' ) == '1' ) {
	...
	/**
	 * Get the URL for viewing poll results.
	 *
	 * @param int $poll_id The poll ID.
	 * @return string
	 */
	public static function get_poll_results_url( $poll_id ) {
		return home_url( "poll/{$poll_id}/results/" );
	}
...
// Initialize the class.
Custom_Endpoints::init();

// Create alias for the Decision_Polls_Custom_Endpoints class for forward compatibility.
if ( ! class_exists( 'Decision_Polls_Custom_Endpoints' ) ) {
	class_alias( 'Custom_Endpoints', 'Decision_Polls_Custom_Endpoints' );
}
