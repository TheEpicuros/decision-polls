...
class Decision_Polls_Poll extends Decision_Polls_Model {
	...
	public function create( $data ) {
		...
		$required_fields = array( 'title', 'answers' );
		...
		if ( ! in_array( $poll_type, self::VALID_POLL_TYPES ) ) {
			return new WP_Error( 'invalid_type', 'Invalid poll type', array( 'status' => 400 ) );
		}
		
		// Validate answers.
		$answers = $data['answers'];
		if ( ! is_array( $answers ) || count( $answers ) < 2 ) {
			return new WP_Error( 'invalid_answers', 'Poll must have at least 2 answers', array( 'status' => 400 ) );
		}
		...
		if ( $existing_poll ) {
			return new WP_Error( 'duplicate_poll', 'A poll with this title already exists. Please use a different title.', array( 'status' => 400 ) );
		}
		...
		if ( ! $poll_id ) {
			return new WP_Error( 'db_error', 'Failed to create poll', array( 'status' => 500 ) );
		}
		...
		// Prepare response.
		$response_data = array_merge( $poll_data, array(
			'id' => $poll_id,
	...
	public function delete( $id ) {
		...
		// Delete poll results.
		$this->wpdb->delete( $results_table, array( 'poll_id' => $id ) );
		
		// Delete votes.
		$this->wpdb->delete( $votes_table, array( 'poll_id' => $id ) );
		
		// Delete answers.
		$this->wpdb->delete( $answers_table, array( 'poll_id' => $id ) );
		
		// Delete poll.
		$deleted = $this->wpdb->delete( $poll_table, array( 'id' => $id ) );
	 ...
	 */
	public function get_user_polls( $user_id, $args = array() ) {
	...
	private function update_answers( $poll_id, $answers ) {
		...
		if ( ! empty( $existing_ids ) ) {
			...
			$this->wpdb->query( $this->wpdb->prepare( 
				"DELETE FROM {$answers_table} WHERE poll_id = %d AND id IN ({$ids_to_delete})",
				$poll_id
			) );
		}
	}
